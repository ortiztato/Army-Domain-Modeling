...
class ArmyUnit {
  constructor(strength, trainingCost, trainingBonus, transformationCost = null) {
...
class Pikeman extends ArmyUnit {
  constructor() { super(5, 10, 3, 30); }
}
class Archer extends ArmyUnit {
  constructor() { super(10, 20, 7, 40); }
}
class Knight extends ArmyUnit {
  constructor() { super(20, 30, 10, null); } // cannot be transformed
}
...
class Army {
  ...
  transformUnit(index) {
    ...
    next.lifeTimeYears = u.lifeTimeYears; // conservar vida
  
    this.units[index] = next;             // reemplaza la unidad
    return next;                          // opcional: devolver la nueva unidad
  }
  ...
  removeTopUnits(n) {
    ...
    return removed.map(u => ({ type: u.constructor.name, strength: u.strength }));
  }
  ...
  battle(opponent) {
    ...
    let result, myGoldChange = 0, oppGoldChange = 0;
    let myLosses = [], oppLosses = [];
...
(function verify() {
  ...
  const count = (army, Cls) => army.units.filter(u => u instanceof Cls).length;
  const printArmy = (name, army) => {
    console.log(`--- ${name} ---`);
    console.log(`Gold: ${army.gold}`);
    console.log(
      `Units: total=${army.units.length} | Pikeman=${count(army, Pikeman)} | Archer=${count(army, Archer)} | Knight=${count(army, Knight)}`
    );
